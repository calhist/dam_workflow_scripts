...
collection=${input%_*}  # drop everything after the "_"

# MODS
for i in $(find ${dir}.MODS -type f | egrep "${dir}.MODS/${collection}_[0-9]+\.xml$"); do
	xmlstarlet val -w $i
	...
	if [[ $i =~ ^.*/(${collection}_[0-9]+)\.xml$ ]]; then
		bag=${BASH_REMATCH[1]}
		mkdir ${output}/${bag}
...
# TEI
for i in $(find ${output} -maxdepth 1 -type d | egrep "${collection}_[0-9]+$"); do
	echo 
	echo $i
